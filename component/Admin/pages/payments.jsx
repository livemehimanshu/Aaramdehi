...
export default function PaymentsPage() {
  const [payments, setPayments] = useState([
    ...
    { id: 'TXN-9003', orderId: 'ORD-103', method: 'UPI', status: 'Failed', amount: 15000 },
  ]);
              ...
              {payments.map((pay) => (
                <tr key={pay.id} className="hover:bg-gray-800/30 transition-all text-sm">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tighter ${getStatusStyle(pay.status)}`}>
                      {pay.status === 'Paid' && <CheckCircle className="inline mr-1" size={10} />}
                      {pay.status === 'Pending' && <Clock className="inline mr-1" size={10} />}
                      {pay.status === 'Failed' && <XCircle className="inline mr-1" size={10} />}
                      {pay.status}
                    </span>
                  </td>
                </tr>
