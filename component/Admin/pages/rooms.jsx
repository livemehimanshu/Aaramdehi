    ...
    <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div key={room._id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-500">
              ...
              <div className="p-6">
                ...
                {room.products && room.products.length > 0 && (
                  <p className="text-[9px] font-bold text-gray-400 uppercase mt-2">🛍️ {room.products.length} Products Linked</p>
                )}
              </div>
            </div>
          ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Living Room" 
                      className="w-full px-5 py-3.5 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-[#1a365d] focus:bg-white outline-none transition-all font-bold text-gray-800" />
                  </div>
                  <div className="space-y-2">
                    ...
                    <select name="categorySlug" value={formData.categorySlug} onChange={(e) => setFormData({...formData, categorySlug: e.target.value})}
                      className="w-full px-5 py-3.5 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-[#1a365d] focus:bg-white outline-none transition-all font-bold text-gray-800 appearance-none">
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="description" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} rows="2"
                    placeholder="Short summary..." className="w-full px-5 py-3.5 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-[#1a365d] focus:bg-white outline-none transition-all font-medium text-gray-800" />
                </div>

                {/* ✅ Fixed: Product list cards UI aligned to Premium Light theme */}
                <div className="space-y-3">
                  <div className="flex justify-between items-center px-1">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.15em]">Featured Products ({formData.products.length})</label>
                    <div className="relative">
                       ...
                       <input 
                        ...
                        className="bg-gray-50 border border-gray-100 rounded-full pl-8 pr-4 py-1.5 text-xs focus:bg-white focus:border-[#1a365d] outline-none w-48 font-medium text-gray-800" 
                       />
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-2xl border border-gray-100 p-2 max-h-48 overflow-y-auto">
                    <div className="grid grid-cols-1 gap-1">
                      {selectableProducts.length === 0 ? (
                        <p className="text-[10px] uppercase font-bold text-gray-400 text-center py-4">No products found</p>
                      ) : (
                        selectableProducts.map(product => (
                          <div 
                            key={product._id} 
                            onClick={() => toggleProductSelection(product._id)}
                            className={`flex items-center gap-3 p-2.5 rounded-xl cursor-pointer transition-all ${formData.products.includes(product._id) ? 'bg-blue-50/80 border border-blue-100' : 'hover:bg-white border border-transparent'}`}
                          >
                            <img src={product.thumbnail || product.images?.[0] || 'https://placehold.co/50'} className="w-9 h-9 object-cover rounded-lg bg-gray-200" alt="" />
                            <div className="flex-1">
                              <p className="text-xs font-bold text-gray-800 line-clamp-1">{product.name}</p>
                              <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{product.brand} • ₹{product.sellingPrice}</p>
                            </div>
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${formData.products.includes(product._id) ? 'bg-[#1a365d] border-[#1a365d]' : 'border-gray-300'}`}>
                ...
                <button type="submit" disabled={isSubmitting} className="w-full bg-[#1a365d] text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-black active:scale-[0.98] transition-all disabled:bg-gray-300 shadow-xl shadow-blue-900/10 text-xs">
