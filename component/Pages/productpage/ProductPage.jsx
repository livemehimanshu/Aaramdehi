...
const ProductPage = (props) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)]">
        <div className="grid gap-8 p-4 sm:p-6 lg:grid-cols-[1.1fr_0.9fr] lg:p-8">
            ...
            <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-3 sm:p-4">
              <div className="relative overflow-hidden rounded-[20px] bg-white" ref={mainImageRef}>
                <img
                  ...
                  alt={titleLabel}
                  className="h-[360px] w-full object-contain sm:h-[460px] lg:h-[560px]"
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-1">
              {galleryImages.map((imgUrl, index) => (
                <button
                  type="button"
                  key={`${imgUrl}-${index}`}
                  className={`h-20 w-20 shrink-0 overflow-hidden rounded-2xl border bg-white transition ${selectedImage === imgUrl ? 'border-orange-500 ring-2 ring-orange-200' : 'border-slate-200 hover:border-slate-300'}`}
                  onClick={() => {
                    if (props.onActiveImgChange) {
              ...
              {currentModelUrl ? (
                <div className="overflow-hidden rounded-[20px] border border-slate-200 bg-white p-2">
                  <model-viewer
                    ...
                    touch-action="pan-y"
                    auto-rotate
                    className="h-[280px] w-full rounded-[16px] bg-slate-100"
                  />
                </div>
              ) : (
                <div className="flex h-[280px] flex-col items-center justify-center rounded-[20px] border border-dashed border-slate-300 bg-white px-6 text-center">
          ...
          <aside className="space-y-5">
            <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <h1 className="mt-4 text-2xl font-semibold tracking-tight text-slate-900 sm:text-[28px]">{titleLabel}</h1>
