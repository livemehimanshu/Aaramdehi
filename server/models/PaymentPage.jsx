...
const PaymentPage = () => {
    ...
    const { orderPayload, totalAmount } = location.state || {}; // CheckoutPage से भेजा गया orderPayload और totalAmount
    ...
    const handlePlaceOrder = async () => {
        ...
        const finalOrderPayload = {
            ...orderPayload,
            paymentMethod: selectedMethod, // सीधे selectedMethod भेजें
        };

        setLoading(true); // Loading state on

        // Simulate UPI/Card/Netbanking payment processing delay
        if (selectedMethod !== 'cod') {
            // Real-world scenario में यहाँ Gateway (Razorpay/Stripe) खुलेगा
            setTimeout(async () => {
                await sendOrderToBackend(finalOrderPayload, token);
            }, 2000); // 2 second delay for simulated payment
        } else {
            // For COD, directly send to backend
            await sendOrderToBackend(finalOrderPayload, token);
        }
    };
    ...
    const sendOrderToBackend = async (payload, token) => {
        try {
            ...
            if (response.data.success) {
                ...
                navigate("/order-success", { state: { orderDetails: response.data.data } }); // Pass order details to success page
            } else {
                alert(response.data.message || "Order placement failed.");
            }
        } catch (error) {
            console.error("Order API Error:", error);
            alert(error.response?.data?.message || "Failed to connect to server. Please try again.");
        } finally {
    ...
    }; // Removed unused `prices` from destructuring
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                errors.cardNumber ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                            }`}
                                        />
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                errors.cardHolder ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                            }`}
                                        />
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                    errors.expiryDate ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                                }`}
                                            />
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                    errors.cvv ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                                }`}
                                            />
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                errors.upi ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                            }`}
                                        />
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className={`w-full border rounded px-4 py-3 text-sm focus:outline-none ${
                                                errors.bank ? 'border-red-500' : 'border-gray-300 focus:border-red-500'
                                            }`}
                                        >
                            ...
                            {selectedMethod === 'emi' && (
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        {[3, 6, 9].map(months => (
                                            <div key={months} className="border border-gray-300 rounded p-3 text-center">
                                                <p className="font-bold text-gray-900">{months} Months</p>
                                                <p className="text-sm text-red-500 mt-1"> {/* Use totalAmount from state */}
                                                    ₹{Math.ceil((totalAmount || 0) / months)}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-green-50 border border-green-200 rounded p-4">
                                        ...
                                        <p className="text-sm text-green-600 mt-2">
                                            You can pay the amount ₹{totalAmount || 0} when the package is delivered to you.
                                        </p>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="bg-yellow-400 rounded-lg p-6 shadow-sm">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    className="bg-gray-900 text-white px-8 py-3 rounded font-bold uppercase tracking-widest hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                >
